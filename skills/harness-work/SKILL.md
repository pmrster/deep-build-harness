...
3. Dispatch the harness-worker subagent (Agent tool, subagent_type harness-worker) with a prompt naming TASK_ID. Wait for its summary.
4. Set status to "submitted".
5. Write "auditor" to state/.active_role.
6. Dispatch the harness-auditor subagent (subagent_type harness-auditor) with TASK_ID. Read its returned verdict, then read the latest matching entry in state/audit_log.json — that entry is authoritative, not the subagent's prose.
...
2. Dispatch the harness-integration subagent (subagent_type harness-integration).
